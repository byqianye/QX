...
import { sourceDisplayLabel, sourceDisplayStatus, type SourceDisplayStatus } from "../source-status.js";
...
function statusFor(siteKey: string): SourceDisplayStatus {
...
function selectSource(siteKey: string): void {
  if (siteKey === selectedKey.value && currentStatus.value === "ready") return;
...
<template>
  <section class="core-page source-switch-page" data-testid="core-source-switch-page">
    ...
    <EmptyState v-if="sites.length === 0" title="暂无已导入来源" message="请先完成配置导入。" :show-action="false" />
    <div v-else class="source-card-grid">
        <article v-for="site in sites" :key="`${sourceId ?? 'unresolved'}-${site.key}`" class="panel source-card" :data-site-key="site.key" :data-current="site.key === selectedKey">
        ...
        <button
          v-if="site.key !== selectedKey || currentStatus !== 'ready'"
          type="button"
          class="button-secondary"
          data-action="core-select-source"
          @click="selectSource(site.key)"
        >{{ site.key === selectedKey ? "重新连接" : "选择来源" }}</button>
      </article>
    </div>
