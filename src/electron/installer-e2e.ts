...
const setup = join(process.cwd(), "dist", "installer", "QX影视-0.1.0-setup.exe");
if (!existsSync(setup)) {
  throw new Error("Installer artifact is missing; run npm run electron:installer:win first");
}
...
try {
  ...
  const removed = await waitForRemoval([installDirectory, desktopShortcut, startMenuDirectory]);
  if (uninstall.code !== 0 || !removed) {
    throw new Error(`Installer uninstall failed: ${JSON.stringify({
      uninstall,
      removed,
      remaining: [installDirectory, desktopShortcut, startMenuDirectory].filter((path) => existsSync(path)),
    })}`);
  }
